...
document.getElementById("inputButtonT").addEventListener('click',function(){
    processSearchType(document.getElementById('inputT').value);
    
});
document.getElementById("inputButtonP").addEventListener('click',function(){
...
function processSearchType(searchId){
    api.searchProductsByType(searchId).then(function(val){
        return Promise(api.searchProductsByPrice(val.price,50));
    }).then(function(val){
        var similarArray = getIntersection(val[0],val[1],val[2].id);
        updateExaminedText(val[2]);
        updateTable('similarTable',similarArray);
...
function processSearchPrice(searchId){
    api.searchProductsByPrice(searchId).then(function(val){
        return Promise.all([api.searchProductsByPrice(val.price,50),api.searchProductsByType(val.type),val]);
    }).then(function(val){
        var similarArray = getIntersection(val[0],val[1],val[2].id);
        updateExaminedText(val[2]);
